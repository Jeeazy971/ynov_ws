const mongodbOperations = require('../db/mongodb/mongodbOperations');

// Créer un nouveau masque
exports.createMask = async (req, res) => {
    try {
        const { name, description, maskJson } = req.body;
        const newMask = await mongodbOperations.createMask({
            name,
            description,
            maskJson,
        });
...
exports.readMasks = async (req, res) => {
    try {
        const masks = await mongodbOperations.readMasks();
...
exports.readMaskById = async (req, res) => {
    try {
        const { id } = req.params;
        const mask = await mongodbOperations.readMaskById(id);
        if (mask) {
...
exports.updateMask = async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, maskJson } = req.body;
        const updatedMask = await mongodbOperations.updateMask(id, {
            name,
            description,
            maskJson,
        });
...
exports.deleteMask = async (req, res) => {
    try {
        const { id } = req.params;
        await mongodbOperations.deleteMask(id);
        res.status(204).send();
    } catch (error) {
        res.status(500).json({
